<template>
    <div class="bewegung-container">
        ...
        <div class="top">
            <div class="top-1">
                <span><b>Lagerungsintervall: tags-alle 2-3 Stunden und auf Wunsch</b></span>
                <span><b>nachts- 3 Std.</b></span>
                <span><b>Dekubitusrisiko - Einschätzung: siehe Braden-Skala</b></span>
            </div>
            <hr>
            <div class="top-2">
                <b>Legende: KP = </b> Kontsdf sdf sg sgfg sgf sgn ljsgn shg lsjgn lsjgn lsjn sjgn ksjgn ksjgn kjsgn ksjgn ksfj sgfsgsgsgsg sg sg 
                sgj nskgj nskgnj ksgn sfjg ksfgn ksfjgn ksfng ksfjgn ksjng kjsng kjsg kjsfng ksfnfg ksjfgn ksfgf kjsfgfn sgj ksfgn sfng  sg sg 
                sgj nskg nksgjn ksgn ksfgn ksfgn ksfgn kjsfgn kjsfjgn ksfgn ksfgn ksfgn kjsfjgn ksfgn kjsfgn ksfgn ksfjgn ksfg nksf nksfgn 
            </div>
            <hr>
            <div class="top-2">
                sgf sg sg sg sg sg sg g s sg sg sg 
                sg sg sg sfg gsgsgf  dfbdsfb    sfg sg sg sg sg sg sfg sfg sfgsgsgfsg sf sd fb dfbsdfbsdfb dsb sd fb sdf b dfb sd fb sdf bsd fb sd fb
                f bds fb sdf bds fb sdf b sdfb sd fb sdf bd fb d fb df bd fb sd fb sdfb dsb
            </div>
        </div>
        <form @submit="onSubmit">
            <table class="styled-table">
                ...
                <tbody>
                    <tr v-for="bewegung in visibleBewegung"
                                :visibleBewegung="visibleBewegung"
                                :bewegung="bewegung"
                                :currentPage="currentPage"
                                :key="bewegung._id"
                                :style="[bewegung.isReport ? 'background-color: rgb(241 84 84 / 32%)' : '']">
                            <td>{{bewegung.datum}}</td>
                            <td>{{bewegung.uhrzeit}}</td>
                            <td v-for="(b, index) in bewegung.xButtons" :key="index">
                                <span v-if="b" style="font-weight: 700">X</span>
                            </td>
                            <td>{{bewegung.bemerkungen}}</td>
                            <td>{{bewegung.nurse}}</td>
                            <td v-if="!overview" style="display: flex; justify-content: center">
                                <svg v-if="nurse.isAdmin" @click="openDeleteModal(bewegung._id)" class="action-icon" viewBox="0 0 24 24"><g fill="none"><path d="M24 0v24H0V0h24ZM12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035c-.01-.004-.019-.001-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427c-.002-.01-.009-.017-.017-.018Zm.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093c.012.004.023 0 .029-.008l.004-.014l-.034-.614c-.003-.012-.01-.02-.02-.022Zm-.715.002a.023.023 0 0 0-.027.006l-.006.014l-.034.614c0 .012.007.02.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01l-.184-.092Z"/><path fill="currentColor" d="M14.28 2a2 2 0 0 1 1.897 1.368L16.72 5H20a1 1 0 1 1 0 2l-.003.071l-.867 12.143A3 3 0 0 1 16.138 22H7.862a3 3 0 0 1-2.992-2.786L4.003 7.07A1.01 1.01 0 0 1 4 7a1 1 0 0 1 0-2h3.28l.543-1.632A2 2 0 0 1 9.721 2h4.558Zm3.717 5H6.003l.862 12.071a1 1 0 0 0 .997.929h8.276a1 1 0 0 0 .997-.929L17.997 7ZM10 10a1 1 0 0 1 .993.883L11 11v5a1 1 0 0 1-1.993.117L9 16v-5a1 1 0 0 1 1-1Zm4 0a1 1 0 0 1 1 1v5a1 1 0 1 1-2 0v-5a1 1 0 0 1 1-1Zm.28-6H9.72l-.333 1h5.226l-.334-1Z"/></g></svg>
                                <svg v-if="nurse.isAdmin" @click="editItem(bewegung)" class="action-icon edit" viewBox="0 0 24 24"><path fill="currentColor" d="M5 23.7q-.825 0-1.413-.588Q3 22.525 3 21.7v-14q0-.825.587-1.413Q4.175 5.7 5 5.7h8.925l-2 2H5v14h14v-6.95l2-2v8.95q0 .825-.587 1.412q-.588.588-1.413.588Zm7-9Zm4.175-8.425l1.425 1.4l-6.6 6.6V15.7h1.4l6.625-6.625l1.425 1.4l-6.625 6.625q-.275.275-.637.438q-.363.162-.763.162H10q-.425 0-.712-.287Q9 17.125 9 16.7v-2.425q0-.4.15-.763q.15-.362.425-.637Zm4.275 4.2l-4.275-4.2l2.5-2.5q.6-.6 1.438-.6q.837 0 1.412.6l1.4 1.425q.575.575.575 1.4T22.925 8Z"/></svg>
                                <svg v-if="!nurse.isAdmin" @click="report(bewegung._id)" class="action-icon report" viewBox="0 0 24 24"><path fill="currentColor" d="M15.73 3H8.27L3 8.27v7.46L8.27 21h7.46L21 15.73V8.27L15.73 3zM19 14.9L14.9 19H9.1L5 14.9V9.1L9.1 5h5.8L19 9.1v5.8z"/><circle cx="12" cy="16" r="1" fill="currentColor"/><path fill="currentColor" d="M11 7h2v7h-2z"/></svg>
                                <svg v-if="nurse.isAdmin && bewegung.isReport" @click="adminReport(bewegung._id)" class="action-icon report" viewBox="0 0 24 24"><path fill="currentColor" d="M15.73 3H8.27L3 8.27v7.46L8.27 21h7.46L21 15.73V8.27L15.73 3zM19 14.9L14.9 19H9.1L5 14.9V9.1L9.1 5h5.8L19 9.1v5.8z"/><circle cx="12" cy="16" r="1" fill="currentColor"/><path fill="currentColor" d="M11 7h2v7h-2z"/></svg>
                            </td>
                    </tr>
                    <tr class="input-row" v-if="((currentPage + 1) === totalPages) || currentMustLover || bewegung.length === 0 || page || page === 0">
                            <td><input v-if="isEdit" class="edit-input table-input" v-model="eDate"  type="date"></td>
                            <td></td>
                            <td v-for="n in xButtons.length" :key="n">
                                <Xbutton :number="n" :array="xButtons" @marked="marked" :isEdit="isEdit" style="width: 35px;" /> 
                            </td>
                            <td><input v-model="bemerkungen" placeholder="Bemerkungen" :class="[isEdit ? 'edit-input':'','table-input']" type="text"></td>
        ...
        <Pagination v-if="!isEdit" :protokolArray="bewegung" :currentPage="currentPage" :pageSize="pageSize" 
        ...
        <!-- <div> -->
        <!-- <tr v-for="headers in testHeaders" :key="headers">
            <th>{{headers}}</th>
            <tr v-for="bewegung in visibleBewegung" :key="bewegung._id">
                <td>{{bewegung.bemerkungen}}</td>
            </tr>
        </tr> -->
        <!-- <tr v-for="headers in testHeaders" :key="headers">
...
<script>
import BewegungService from '@/BewegungService'
...
export default {
    ...
    data(){
        return {
            ...
            visibleBewegung: [],
            eDate: '',
            // izbrisati ovo dole sao za testiranje je !
            testHeaders: ['prvi', 'drugi', 'treci', 'cetvrti', 'peti'],
            xButtons: [false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false,false],
            bemerkungen: '',
            //------------------------------------------------------
            openReport: false,
    ...
    async created() {
        this.bewegung = await BewegungService.getSpecificBewegung(this.patient.name)
        this.isLoaded = true

        if(this.overview === 'overview') {
            this.visibleBewegung = await BewegungService.getDataForOverview(this.patient.name,this.von,this.bis)
            this.currentPage = 0
        }
        ...
        if(this.page || this.page === 0) {
            ...
            this.$emit('topDate', this.visibleBewegung)
        } else {
            if(!this.overview) {
                this.currentPage = Math.ceil(this.bewegung.length / this.pageSize) 
                this.updateVisible()
                this.alterCurrentValue()
            }
        }
        this.currentPage === 1 && this.visibleBewegung.length === this.pageSize ? this.hidePrevious = true : this.hidePrevious = false
    },
    methods: {
        async viewAll() {
            this.viewall = !this.viewall
            this.isLoaded = false
            this.bewegung = await BewegungService.getSpecificBewegung(this.patient.name,this.viewall)
            this.isLoaded = true
            this.currentPage = Math.ceil(this.bewegung.length / this.pageSize) 
            this.updateVisible()
            this.alterCurrentValue()
        ...
        updateVisible() {
        this.visibleBewegung = this.bewegung.slice(this.currentPage * this.pageSize, (this.currentPage * this.pageSize) + this.pageSize)
        if(this.visibleBewegung.length === 0 && this.currentPage > 0) {
            this.updatePage(this.currentPage - 1)
        }
        },
        alterCurrentValue() {
            this.totalPages = Math.ceil(this.bewegung.length / this.pageSize)
            if(this.visibleBewegung.length === this.pageSize) {
                this.currentPage++
        ...
        async edit() {
            await BewegungService.editBewegung (
                this.ID,
        ...
        async deleteItem(id) {
            await BewegungService.deleteBewegung(id)
            .then(response => {
            ...
            this.bewegung = await BewegungService.getSpecificBewegung(this.patient.name)
            this.visibleBewegung.length === 1 ? this.onLastItemUploaded = false : this.onLastItemUploaded = true
            this.updateVisible()
            this.onLastItemUploaded = false
            this.alterCurrentValue()
        },
        ...
        async adminReport(id) {
            this.ID = id
            let a = {}
            a = await BewegungService.getReport(id,this.patient.name)
        ...
        async onSubmit(e) {
            e.preventDefault()
            await BewegungService.insertBewegung (
                this.patient.name,
                this.nurse.username,
                this.xButtons,
                this.bemerkungen
