...
//     res.send(await pflegebericht.find({}).toArray())
// })
router.get("/:patientName/:viewall", async (req, res) => {
    let seeAll = (req.params.viewall === "true")
    pflegebericht = await loadPflegeberichtCollection()
    // res.send(await pflegebericht.find({patientName:req.params.patientName ,
    //                         createdAt: { $lt: new Date(),    // ovo getDate()-3 je broj dana u nazad !!!!!
    //                         $gte: new Date(new Date().setDate(new Date().getDate()-7))}}).toArray())
    if(seeAll) {
        let pflegeberichtAll = pflegebericht.find({patientName:req.params.patientName})
        res.send(await pflegeberichtAll.toArray())
    } else {
        let pflegeberichtSeven = pflegebericht.find({patientName:req.params.patientName ,
            createdAt: { $lt: new Date(),    
            $gte: new Date(new Date().setDate(new Date().getDate()-7))}})
            res.send(await pflegeberichtSeven.toArray())
    }
...
router.get("/report/:id/:patientName", async (req, res) => {
    pflegebericht = await loadPflegeberichtCollection()
    res.send(await pflegebericht.find({_id: new mongodb.ObjectId(req.params.id),patientName:req.params.patientName}).toArray())
})
...
router.post("/", async (req, res) => {
    const pflegebericht = await loadPflegeberichtCollection()
    await pflegebericht.insertOne({
...
router.delete("/:id", async (req, res) => {
    const pflegebericht = await loadPflegeberichtCollection()
    await pflegebericht.deleteOne({_id: new mongodb.ObjectId(req.params.id)})
